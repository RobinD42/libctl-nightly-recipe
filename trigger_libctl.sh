...
CRON_DIR=$HOME/cron
# The local directory where you have checkout out Simpetus/libctl-nightly-recipe.git
...
# The name of the file that holds the most recent NanoComp/libctl.git commit hash
COMMIT_FILENAME=latest_libctl_commit.txt
...
LATEST_COMMIT=$(git ls-remote git://github.com/NanoComp/libctl.git refs/heads/master | cut -f 1)
...
if [ "${LAST_KNOWN_COMMIT}" != "${LATEST_COMMIT}" ]; then
    ...
    pushd ${RECIPE_DIR}/libctl-nightly-recipe
    git checkout master
